...
int main(int argc, char** argv){
	...
	VKRenderer::onFileDrop([](vector<string> files){
		...
		// Only one structure at a time, so a multi-file drop is not meaningful here.
		// `file` is scoped locally like the glb branch above: the handler no longer
		// has a single handler-wide `file`, since point clouds accept several at once.
		if(files.size() == 1 && (iEndsWith(files[0], ".cif") || iEndsWith(files[0], ".mmcif"))) {
			string file = files[0];
